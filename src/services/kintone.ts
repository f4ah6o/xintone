import type { KintoneGetRecordsResponse, KintoneGetRecordResponse, KintoneRecord } from '../types';
...
export class KintoneService {
   ...
   * レコード一覧を取得
   */
  async getRecords(apiToken: string, query?: string, fields?: string[]): Promise<KintoneGetRecordsResponse> {
   ...
   * 単一レコードを取得
   */
  async getRecord(apiToken: string, recordId: string): Promise<KintoneGetRecordResponse> {
   ...
   * レコードを追加
   */
  async createRecord(apiToken: string, record: KintoneRecord): Promise<{ id: string; revision: string }> {
